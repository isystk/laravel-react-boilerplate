...
class UpdateServiceTest extends BaseTest
{
    ...
    public function test_update(): void
    {
        ...
        // データが更新されたことをテスト
        $this->assertDatabaseHas('contacts', ['id' => $contact->id, 'user_name' => 'bbb']);
        $this->assertDatabaseHas('contacts', ['id' => $contact->id, 'title' => 'タイトル2']);
        $this->assertDatabaseHas('contacts', ['id' => $contact->id, 'email' => 'bbb@test.com']);
        $this->assertDatabaseHas('contacts', ['id' => $contact->id, 'url' => 'https://bbb.test.com']);
        $this->assertDatabaseHas('contacts', ['id' => $contact->id, 'gender' => Gender::Female->value]);
        $this->assertDatabaseHas('contacts', ['id' => $contact->id, 'age' => Age::Over40->value]);
        $this->assertDatabaseHas('contacts', ['id' => $contact->id, 'contact' => 'お問い合わせ2']);

        // 新しい画像レコードが作成されたことをテスト
        $updatedContact = $contact->fresh();
        $image2         = Image::find($updatedContact->image_id);
