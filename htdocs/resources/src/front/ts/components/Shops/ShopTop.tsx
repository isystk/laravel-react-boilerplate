          ...
          <div className="text-right mb-2">
            <a
              ...
              onClick={e => {
                ...
                if (stock.isLike) {
                  ;(async () => {
                    const response = await API.post(`${API_ENDPOINT.LIKES_DESTROY}/${stock.id}`)
                    if (response.result) {
                      dispatch({ type: 'REMOVE_LIKE', id: stock.id })
                    }
                  })()
                } else {
                  ;(async () => {
                    const response = await API.post(API_ENDPOINT.LIKES_STORE, { id: stock.id })
                    if (response.result) {
                      window.alert('お気に入りに追加しました')
                      dispatch({ type: 'ADD_LIKE', id: stock.id })
                    }
                  })()
                }
              }}
